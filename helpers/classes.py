...
from lrgb_synth.encoders.composition import Concat2NodeEncoder
...
class ActivationType(Enum):
    ...
    def get(self):
        if self is ActivationType.RELU:
            ...
            return F.gelu
        elif self is ActivationType.ID:
            return Identity()
        else:
...
class GNNArgs(NamedTuple):
    ...
    def load_net(self) -> ModuleList:
        if self.pos_enc is PosEncoder.NONE:
            enc_list = [
                self.dataset_encoders.node_encoder(
                    in_dim=self.in_dim, emb_dim=self.hidden_dim
                )
            ]
        else:
            if self.dataset_encoders is DataSetEncoders.NONE:
                enc_list = [
                    self.pos_enc.get(in_dim=self.in_dim, emb_dim=self.hidden_dim)
                ]
            else:
                enc_list = [
                    Concat2NodeEncoder(
                        enc1_cls=self.dataset_encoders.node_encoder,
        ...
        component_list = self.model_type.get_component_list(
        ...
        if self.dec_num_layers > 1:
            mlp_list = (self.dec_num_layers - 1) * [
                Linear(self.hidden_dim, self.hidden_dim),
                Dropout(self.dropout),
                self.act_type.nn(),
            ]
            mlp_list = mlp_list + [Linear(self.hidden_dim, self.out_dim)]
            dec_list = [Sequential(*mlp_list)]
        else:
            dec_list = [Linear(self.hidden_dim, self.out_dim)]
