...
from lrgb_exps.graphgps.loss.weighted_cross_entropy import (
...
class MetricType(Enum):
    ...
    def apply_metric(self, scores: np.ndarray, target: np.ndarray) -> float:
        ...
        if self is MetricType.ACCURACY:
            ...
            metric = AveragePrecision(task="multilabel", num_labels=num_classes).to(
                scores.device
            )
        ...
        elif self is MetricType.F1_MULTICLASS:
    ...
    def is_classification(self) -> bool:
        if self in [
            MetricType.AUC_ROC,
            MetricType.ACCURACY,
            MetricType.MULTI_LABEL_AP,
            MetricType.F1_MULTICLASS,
        ]:
            return True
        elif self is MetricType.MSE_MAE:
            return False
    ...
    def weighted_classes(self) -> bool:
        return self in [MetricType.F1_MULTICLASS]
    ...
    def get_out_dim(self, dataset: List[Data]) -> int:
        """
        Determines the output dimension of the dataset based on the type of task (classification or regression).
        Args:
            dataset (List[Data]): A list of data objects, where each data object contains a 'y' attribute representing the target values.
        Returns:
            int: The output dimension of the dataset. For classification tasks, it returns the number of classes.
                 For multilabel classification, it returns the number of labels. For regression tasks, it returns the dimension of the target variable.
        """
        ...
        if self.is_classification():
            ...
            return dataset.get(0).y.shape[-1]
    ...
    def get_worst_losses_n_metrics(self) -> LossesAndMetrics:
        if self.is_classification():
            return LossesAndMetrics(
                train_loss=math.inf,
                val_loss=math.inf,
                test_loss=math.inf,
                train_metric=-math.inf,
                val_metric=-math.inf,
                test_metric=-math.inf,
            )
        else:
            return LossesAndMetrics(
                train_loss=math.inf,
