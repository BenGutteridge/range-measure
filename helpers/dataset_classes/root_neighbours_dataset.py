...
class RootNeighboursDataset(object):
    ...
    def generate_component(self) -> Data:
        ...
        train_mask, val_mask, test_mask = self.mask_task(
            num_nodes_per_fold=num_nodes_per_fold
        )
    ...
    def initialize_constants(self) -> Dict[str, int]:
        return {
            "NUM_COMPONENTS": 1000,
            "MAX_HUBS": 3,
            "MAX_1HOP_NEIGHBORS": 10,
            "ADD_HUBS": 2,
            "HUB_NEIGHBORS": 5,
            "MAX_2HOP_NEIGHBORS": 3,
            "NUM_FEATURES": 5,
        }
    ...
    def generate_fold(self, eval: bool) -> Data:
        ...
        num_hubs = (
            torch.randint(1, MAX_HUBS + 1, size=(1,), generator=self.generator).item()
            + add_hubs
        )
        num_1hop_neighbors = torch.randint(
            MAX_HUBS + add_hubs,
            MAX_1HOP_NEIGHBORS + 1,
            size=(1,),
            generator=self.generator,
        ).item()
        assert num_hubs <= num_1hop_neighbors

        list_num_2hop_neighbors = torch.randint(
            1,
            MAX_2HOP_NEIGHBORS,
            size=(num_1hop_neighbors - num_hubs,),
            generator=self.generator,
        ).tolist()
        ...
        for num_2hop_neighbors in list_num_2hop_neighbors:
            ...
            if num_2hop_neighbors > 0:
                clique_edge_index = torch.tensor(
                    [[0] * num_2hop_neighbors, list(range(1, num_2hop_neighbors + 1))]
                )
