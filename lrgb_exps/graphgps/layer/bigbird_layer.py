...
class BigBirdBlockSparseAttention(nn.Module):
    def __init__(self, config, seed=None):
        ...
        self.query = nn.Linear(
            config.dim_hidden, self.all_head_size, bias=config.use_bias
        )
        self.key = nn.Linear(
            config.dim_hidden, self.all_head_size, bias=config.use_bias
        )
...
class BigBirdIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(
            config.dim_hidden, config.dim_hidden
        )  # dim_hidden -> intermediate_size
...
class BigBirdModel(nn.Module):
    ...
    def _pad_to_block_size(
        ...
        if padding_len > 0:
            ...
            inputs_embeds_padding = torch.zeros(
                (batch_size, padding_len, emb_dim),
                dtype=torch.float,
...
class SingleBigBirdLayer(BigBirdModel):
    ...
    def __init__(self, config):
        ...
        ), "SingleBigBirdLayer is configured to have sparse attention"
        config.layers = 1
