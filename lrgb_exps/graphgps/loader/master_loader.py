...
from lrgb_exps.graphgps.loader.dataset.aqsol_molecules import AQSOL
from lrgb_exps.graphgps.loader.dataset.coco_superpixels import COCOSuperpixels
from lrgb_exps.graphgps.loader.dataset.malnet_tiny import MalNetTiny
from lrgb_exps.graphgps.loader.dataset.voc_superpixels import VOCSuperpixels
from lrgb_exps.graphgps.loader.split_generator import (
    prepare_splits,
    set_dataset_splits,
)
from lrgb_exps.graphgps.transform.posenc_stats import compute_posenc_stats
from lrgb_exps.graphgps.transform.task_preprocessing import (
    task_specific_preprocessing,
)
from lrgb_exps.graphgps.transform.transforms import (
...
from longrange.task_preprocessing import add_distance_attrs
...
def preformat_OGB_Graph(dataset_dir, name):
    ...
    if name == "ogbg-ppa":
        ...
        from lrgb_exps.graphgps.loader.ogbg_code2_utils import (
            idx2vocab,
            get_vocab_mapping,
            augment_edge,
            encode_y_to_arr,
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary
        from lrgb_exps.graphgps.loader.dataset.pcqm4mv2_contact import (
...
def convert_x_to_float32(data: Data) -> Data:
    """
    Converts the .x attribute of a PyG Data object from integers to float32.

    Args:
        data (Data): A PyG Data object with an .x attribute.

    Returns:
        Data: The modified Data object with .x converted to float32.
    """
    if data.x is not None and data.x.dtype.is_floating_point is False:
        data.x = data.x.to(torch.float32)
    return data


def to_one_hot(data: Data) -> Data:
    """
    Converts integer input features for Peptides dataset to one-hot so they are differentiable.
    """

    feature_dims = get_atom_feature_dims()
    # x: [num_nodes, len(feature_dims)], integer node features
    x_cat = []
    for i, dim in enumerate(feature_dims):
        x = data.x[:, i]
        x = torch.nn.functional.one_hot(x, num_classes=dim).squeeze().float()
        x_cat.append(x)
    data.x = torch.cat(x_cat, dim=-1)  # -> [num_nodes, sum(feature_dims)], one-hot
    return data
...
def preformat_Peptides(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary.
        from lrgb_exps.graphgps.loader.dataset.peptides_functional import (
            PeptidesFunctionalDataset,
        )
        from lrgb_exps.graphgps.loader.dataset.peptides_structural import (
    ...
    if cfg.longrange.track_range_measure:
        # Pre-transform for distance features
        pre_transform = lambda data: add_distance_attrs(
            add_distance_attrs(data, "resistance"), "spd"
        )  # both resistance and spd
        dataset_dir = osp.join(dataset_dir, "+resistance+spd")
        # Transform for differentiable one-hot encoding
        if not cfg.dataset.node_encoder_name.startswith("AtomDiff"):
            cfg.dataset.node_encoder_name = cfg.dataset.node_encoder_name.replace(
                "Atom", "AtomDiff"
            )
        transform = to_one_hot
        print(
            "Converting integer node features to one-hot floats for differentiability."
        )
