...
from lrgb_exps.graphgps.encoder.graphormer_encoder import graphormer_pre_processing
...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    if "RWSEEdge" in pe_types:
        ...
        data = data.to_heterogeneous(
            node_type_names=["V"], edge_type_names=[("V", "E", "V")]
        )
        if data.num_nodes == 1:
            data.y = data["V"].y
            del data["V"].y
    ...
    if "GraphormerBias" in pe_types:
        data = graphormer_pre_processing(
            data, cfg.posenc_GraphormerBias.num_spatial_types
        )
...
def get_edge_rw_landing_probs(
    ksteps, edge_index, edge_weight=None, num_nodes=None, num_global=4
):
    """Compute Random Walk landing probabilities for given list of K steps.
    ...
    deg = scatter(
        edge_weight, src, dim=0, dim_size=num_nodes, reduce="sum"
    )  # Out degrees.
    deg_inv = deg.pow(-1.0)
    deg_inv.masked_fill_(deg_inv == float("inf"), 0)
    ...
    if ksteps == list(range(min(ksteps), max(ksteps) + 1)):
        ...
        )  # P.clone().detach().matrix_power(min(ksteps))
        P_list.append(Pk)
        for k in range(min(ksteps), max(ksteps) + 1):
            Pk = Pk @ P
            P_list.append(Pk)
