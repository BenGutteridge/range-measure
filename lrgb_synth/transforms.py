...
from lrgb_synth.encoders.compute import compute_posenc_stats
from lrgb_synth.split_generator import set_dataset_splits
...
def apply_transform(dataset, pos_encoder):
    ...
    dataset = pre_transform_in_memory(
        dataset,
        partial(
            compute_posenc_stats, pos_encoder=pos_encoder, is_undirected=is_undirected
        ),
        show_progress=True,
    )
