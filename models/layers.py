...
class MolConv(MessagePassing):
    def message(
        self, x_j: Tensor, edge_attr: OptTensor, edge_weight: OptTensor = None
    ) -> Tensor:
...
class WeightedGCNConv(MolConv):
    ...
    def forward(
        ...
        _, edge_attr = add_remaining_self_loops(
            edge_index, edge_attr, fill_value=1, num_nodes=x.shape[0]
        )
        ...
        out = self.propagate(
            edge_index, x=x, edge_weight=edge_weight, size=None, edge_attr=edge_attr
        )
        out = self.lin(out)
        return out
...
class WeightedGINConv(MolConv):
    def __init__(
        self, in_channels: int, out_channels: int, bias: bool, mlp_func: Callable
    ):
        """
...
# model_type={SUM_GNN, MEAN_GNN}
class WeightedGNNConv(MolConv):
    def __init__(
        self, in_channels: int, out_channels: int, aggr: str, bias: bool, **kwargs
    ):
